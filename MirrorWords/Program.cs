...
namespace MirrorWords
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            Regex pattern = new Regex(@"(@|#)(?<first>[A-Za-z]{3,})\1\1(?<second>[A-Za-z]{3,})\1"); //#Part##traP#  @leveL@@Level@  #sAw##wAs#

            MatchCollection words = pattern.Matches(input);
            int matchCounter = 0; //counter
            ...
            foreach (Match match in words)
            {
                if (match.Success)
                {
                    ...
                    string secondReversed = new string(secondToChar); //char array to string (again)
